...
#ifndef __SIMPHBKSTORE_H_
#define __SIMPHBKSTORE_H_
...
const TInt KPhBkMaxAlphaTagSize=241;
const TInt KPhBkMaxTelNumSize=100;
...
class CSimPhBkStore : public CSimPhone, public MTimerCallBack
	{
public:
	static CSimPhBkStore* NewL(CSimPhone* aPhone, const TDesC8& aName, TInt aMaxNumSlots, TInt aMaxNumLen, TInt aMaxTextLen);
	CSimPhBkStore(CSimPhone* aPhone);
	~CSimPhBkStore();
...
//coverity[bad_override]    	
	...
	TInt MaxSlots();
protected:
	void ConstructL(const TDesC8& aName, TInt aMaxNumSlots, TInt aMaxNumLen, TInt aMaxTextLen);
...
private:
	...
	TInt GetPhBkEntry(const CTestConfigItem* aItem, TInt aTagIndex, TPtrC8& aPhonebookName,
					  TInt& aIndex, TPtrC8& aTelNum, TPtrC8& aAlphaTag, TUint8& aNpiTon);
...
private:
	...
	TPhBkStoreEntry* iPhBkStoreEntries;	// < A pointer to the Phonebook Store array.
	...
	TPhBkStoreEntry iPhBkOOBWrite;			// < The phonebook entry to be written in the "out of band" write.
