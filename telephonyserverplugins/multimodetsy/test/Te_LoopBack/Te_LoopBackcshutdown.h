...
#ifndef __CSHUTDOWN_H__
#define __CSHUTDOWN_H__

#include "Te_LoopbackTestStepBase.h"
#include "Te_LoopBackScriptEng.h"

const TUint8 KTestShutDownTxPduA[]= { 0x0B,0x91,0x44,0x83,0x05,0x61,0x00,0xF5,
...
const TUint KShutdownMsgRefA=25;

const struct TScript KShutdownScript[]= {
...
// Init Complete
	...
	{	ERxString, (TText8*)"AT+CNMI=?",	0, ERxEvent	},
	{	ETxString, (TText8*)"+CNMI: (0-1),(0,1,3),(0),(0),(0,1)\r",	0, ETxEvent|EFailIfTimeOut	},
	{	ETxStringOK, NULL,					0, ETxEvent|EFailIfTimeOut	},

	{	ERxString, (TText8*)"AT+CNMI=1,1",	0, ERxEvent	},
	{	ETxStringOK, NULL,					0, ETxEvent|EFailIfTimeOut	},
...
const struct TScript KShutdownScriptA[]= {
	{	ERxString,	(TText8*)"AT\r",		0, ERxEvent	},
	{	ETxStringOK, NULL,					0, ETxEvent|EFailIfTimeOut	},
...
//
//  Now, wait for the detected networks command, but never completed it.
//
	{	ERxString, (TText8*)"AT+COPS=?",	0, ERxEvent	},
...
class CTestDriveShutdown : public CTestBase
	{
public:
	static CTestDriveShutdown* NewL(const TScriptList aScriptListEntry);
...
private:
	CTestDriveShutdown(const TScriptList aScriptListEntry);

	TScriptList iScriptListEntry;
	};

class CTestShutdown : public CATScriptEng
	{
public:
	static CTestShutdown* NewL(const TScript* aScript);
	virtual void SpecificAlgorithmL(TInt aParam);
	virtual void Complete(TInt aError);
	virtual TInt Start();

private:
	CTestShutdown(const TScript* aScript);
	void ConstructL();

	const TScript* iScript;
	};

class CTestDriveShutdownA : public CTestBase
	{
public:
	static CTestDriveShutdownA* NewL(const TScriptList aScriptListEntry);
	virtual TInt RunTestL();
...
private:
	CTestDriveShutdownA(const TScriptList aScriptListEntry);
