...
#ifndef __CSMSCANCEL_H__
#define __CSMSCANCEL_H__

#include "Te_LoopbackTestStepBase.h"
#include "Te_LoopBackScriptEng.h"

//
// Cancel Script using CMT SMS Rx Mode
//
const struct TScript KSmsCancelScript[]= {
...
//
// Cancel Script using CMTI SMS Rx Mode
//
const struct TScript KSmsCancelCmtiScript[]= {
	{	ERxString,	(TText8*)"AT\r",		0, ERxEvent	},
	{	ETxStringOK, NULL,					0, ETxEvent|EFailIfTimeOut	},
...
//
// Rx SMS Cancel messages
//
class CTestDriveSmsCancel : public CTestBase
	{
public:
	static CTestDriveSmsCancel* NewL(const TScriptList aScriptListEntry);
...
private:
	CTestDriveSmsCancel(const TScriptList aScriptListEntry);

	TScriptList iScriptListEntry;
	};

class CTestSmsCancel : public CATScriptEng
	{
public:
	static CTestSmsCancel* NewL(const TScript* aScript);
	virtual void SpecificAlgorithmL(TInt aParam);
	virtual void Complete(TInt aError);
	virtual TInt Start();

private:
	CTestSmsCancel(const TScript* aScript);
	void ConstructL();
