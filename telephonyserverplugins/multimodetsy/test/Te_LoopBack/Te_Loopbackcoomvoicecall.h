...
#ifndef __COOMVOICECALL_H__
#define __COOMVOICECALL_H__
...
//
const struct TScript KOOMVoiceCallScriptA[]= {
...
// Now simulate an outgoing and an incoming call when the behaving like an Ericsson 888
// so the iPhone will go "deaf" for a while after the "ATDxxxx", "OK" response
...
//  Mobile "Prod" removed corresponding to defect fix MIV-55LLFT for Ericsson devices 
//	{	ERxString, (TText8*)"AT+",			0, ERxEvent	},
...
class CTestDriveOOMVoiceCall : public CTestBase
	{
public:
	static CTestDriveOOMVoiceCall* NewL(const TScriptList aScriptListEntry);
	virtual TInt RunTestL();
protected:
	virtual TInt DriveETelApiL();
private:
	CTestDriveOOMVoiceCall(const TScriptList aScriptListEntry);
	void CloseDown();
	void OpenUpL();
	TInt DoTestL(TInt aAllocFailNumber);

private:
	TScriptList iScriptListEntry;
	};

class CTestOOMVoiceCall: public CATScriptEng
	{
public:
	static CTestOOMVoiceCall* NewL(const TScript* aScript);
	virtual void SpecificAlgorithmL(TInt aParam);
	virtual void Complete(TInt aError);
	virtual TInt Start();
private:
	CTestOOMVoiceCall(const TScript* aScript);
	void ConstructL();
