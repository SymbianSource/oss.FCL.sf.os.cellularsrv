...
#ifndef __CVOICECALL_H__
#define __CVOICECALL_H__
...
//
const struct TScript KVoiceCallScriptA[]= {
...
class CTestDriveVoiceCall : public CTestBase
	{
public:
	static CTestDriveVoiceCall* NewL(const TScriptList aScriptListEntry);
...
private:
	CTestDriveVoiceCall(const TScriptList aScriptListEntry);
	TScriptList iScriptListEntry;
	};

class CTestVoiceCall : public CATScriptEng
	{
public:
	static CTestVoiceCall* NewL(const TScript* aScript);
	virtual void SpecificAlgorithmL(TInt aParam);
	virtual void Complete(TInt aError);
	virtual TInt Start();

private:
	CTestVoiceCall(const TScript* aScript);
	void ConstructL();
