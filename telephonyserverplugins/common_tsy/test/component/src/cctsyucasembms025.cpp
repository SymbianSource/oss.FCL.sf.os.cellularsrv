...
CTestSuite* CCTsyUCaseMbms025::CreateSuiteL(const TDesC& aName) 
	{
	SUB_SUITE;        
    ADD_TEST_STEP_ISO_CPP(CCTsyUCaseMbms025, CCtsyIntegrationTestMbms0001L );
	END_SUITE;
	}
 ...
 *
 * Reason for test: Verify that client gets notification about MBMS session changes.
 *
 * @return - none
 */
void CCTsyUCaseMbms025::CCtsyIntegrationTestMbms0001L()
    {
	...
	// The Client posts notification RPacketContext::NotifyContextStatusChange to receive status changes for 
	// MBMS context ETEL API posts notification to receive change in MBMS context session from CTSY.
    ...
    // After a time Session ends 
    User::After(1000);

    // CTSY completes context status EPacketContextNotifyStatusChange EStatusActive
	mbmsContext.NotifyStatusChange(requestStatus, contextStatus);	
	...
	AssertMockLtsyStatusL();
	CleanupStack::PopAndDestroy( 7, this ); // sessionList, session, data, expectData, mbmsContext, packetservice, this
      }
